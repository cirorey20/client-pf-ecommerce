...
export function Orders() {
    ...
    const { orders: allOrders, states } = useSelector((state) => state.ordersReducer);
            ...
            <div>
                ...
                <div className="overflow-x-auto grid grid-cols-5">
                    ...
                    <div className="col-start-5 col-end-6 row-start-1 row-end-2 py-4 place-self-center ">
                        <input type="search" placeholder="Search here ..." onKeyDown={onSearch} className="rounded-full text-white placeholder:text-gray-300 bg-[#644b9c] border-none focus:ring-transparent" />
                    </div>
                        ...
                        <div className="rounded text-center block text-white bg-[#644b9c] border-none focus:ring-transparent p-3">
                            <div className="flex items-center justify-center">
                                <div className="datepicker relative form-floating mb-3 xl:w-96" data-mdb-toggle-button="false">
                                    <label htmlFor="floatingInput" className="text-white">From</label>
                                    <input type="date"
                            ...
                            <div className="flex items-center justify-center">
                                <div className="datepicker relative form-floating mb-3 xl:w-96" data-mdb-toggle-button="false">
                                    <label htmlFor="floatingInput" className="text-white">To</label>
                                    <input type="date"
                                        onChange={onChangeDate}
                                        name="dateTo"
                                        value={date.dateTo}
                            ...
                            <button onClick={onClickFilter} className="px-3 bg-green-600 rounded-full">Flter</button>
                            <button onClick={onClickFilterReset} className="px-3 bg-green-600 rounded-full">Reset</button>
                        </div>
                                ...
                                save?.map(o => (
                                    <tr key={o.id} className="bg-[#f1eff0]">
                                        ...
                                        <td className="py-6 px-6">
                                            <select value={o.state} onChange={e => onChangeState(e, o.id)} className="block text-white bg-[#644b9c]  px-6 rounded-full  border-none focus:ring-transparent">
                                            ...
                                            {save?.find(s => s.id === o.id)?.isChange && <button onClick={() => onClickSave(o.id)} className="rounded bg-green-500 p-1 mt-0.5">SAVE</button>}
                                        </td>
                                        <td className="py-6 px-6 text-black">
                                            {
                                                `${new Date(o.date).getDate()}-${new Date(o.date).getMonth() + 1}-${new Date(o.date).getFullYear()} ${new Date(o.date).getHours()}:${new Date(o.date).getMinutes()}`
                                            }
                                        </td>
