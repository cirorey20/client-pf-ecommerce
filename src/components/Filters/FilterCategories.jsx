...
export default function FilterCategory({ allCategories }) {
    ...
    function onClickFilter() {
        ...
        oldUrl = oldUrl.join('&');
        oldUrl = (oldUrl[0] === '&') ? `?${oldUrl.slice(1)}` : (oldUrl[0] !== '?') ? `?${oldUrl}` : oldUrl;
    ...
    function onClickReset(){
        const newState = checksCategory.map(category => ({...category, checked:false}));
        setChecksCategory([...newState])
    ...
    return (
        <div>
            ...
            <button type="button" onClick={onClickFilter} className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Filter</button>
            <button type="button" onClick={onClickReset} className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">Reset</button>
